...
export function WeatherWidget({ weather }: WeatherWidgetProps) {
  return (
    <Card className="animate-slideUp bg-white/95 dark:bg-neutral-900/95">
      <h3 className="mb-4 text-lg font-semibold tracking-tight">5-Day Forecast</h3>
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
        {weather.map((day) => {
          const Icon = iconForCategory(day.category);
          return (
            <div key={day.date} className="rounded-xl border border-gray-200 bg-gray-50 p-3 text-sm dark:border-gray-700 dark:bg-neutral-800">
              ...
              <p className="mt-2 text-xs text-gray-600 dark:text-gray-300">Rain chance: {(day.maxPop * 100).toFixed(0)}%</p>
            </div>
          );
        })}
      </div>
